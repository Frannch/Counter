...
const GAME_ID = 'dice'           // Cambia a un id existente en js/lobby.js (p.ej. 'dice', 'rompe', etc.)
  ...
  const bulletIntervalId = setInterval(() => {
    ...
    if(squares[fireIndex].classList.contains('alien1')) {
      ...
      if (aliens.length === 0) {
        ...
        // Quitar oferta de “Next Level” y botonera
        // startButton.innerText = ''
        // startButton.innerText = 'Next Level'
        // startButton.addEventListener('click', startGame)
        // startButton.classList.remove('hidden')

        // Mostrar cartel + cuenta regresiva y volver al lobby con ?win=GAME_ID
